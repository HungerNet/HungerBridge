...
public final class PlayerBanHandler implements HttpHandler {
    private final Config config;
    private final Logger logger;
    private final CommandExecutor executor;

    public PlayerBanHandler(Config config, Logger logger, CommandExecutor executor) {
        this.config = config;
        this.logger = logger;
    ...
    public void handle(HttpExchange ex) throws IOException {
        ...
        if (!HttpUtil.checkAcl(ex, config, "players.ban")) {
            HttpUtil.error(ex, 403, "forbidden", "Token not permitted to ban players", config);
            return;
        }
        if (!HttpUtil.rateLimit(ex, config, "players.ban")) return;
        ...
        String reason = json.has("reason") ? json.get("reason").getAsString() : "Banned by HungerBridge";
        executor.execute("ban " + player + " " + reason);
