...
public final class WorldTpsHandler implements HttpHandler {
    private final Config config;
    private final Logger logger;
    private final CommandExecutor executor;

    public WorldTpsHandler(Config config, Logger logger, CommandExecutor executor) {
        this.config = config;
        this.logger = logger;
    ...
    public void handle(HttpExchange ex) throws IOException {
        ...
        if (!HttpUtil.checkAcl(ex, config, "world.tps")) {
            HttpUtil.error(ex, 403, "forbidden", "Token not permitted to access TPS", config);
            return;
        }
        if (!HttpUtil.rateLimit(ex, config, "world.tps")) return;

        JsonObject resp = Json.obj("ok", true, "tps", executor.getTps(), "tps_1m", executor.getTps1m(), "tps_5m", executor.getTps5m(), "tps_15m", executor.getTps15m());
